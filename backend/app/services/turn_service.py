...
class _TurnPreparation:
    """Shared pre-flight state for streaming and non-streaming turns."""
...
class TurnService:
    ...
    async def process_turn(self, *, user_id: str, payload: TurnRequest) -> TurnResult:
        ...
        try:
            ...
            (
                ...
                response_text,
                reasoning_content,
            ) = await self._extract_claims(payload, user_id=user_id, turn_id=turn_id, prep=prep)
    ...
    async def process_turn_stream(
        self, *, user_id: str, payload: TurnRequest
        ...
        try:
            ...
            stream = self.llm_router.route_stream(
                ...
                enforce_local=prep.enforce_local,
                messages=messages,
                preferred_provider=prep.preferred_provider,
                preferred_model=prep.preferred_model,
            ...
            if final_response is None:
                raise ServiceError("LLM stream ended without a final response")

            extracted_claims = await self.claim_extractor.extract(
                llm_response_text=final_response.content,
    ...
    async def _prepare_turn(self, *, user_id: str, payload: TurnRequest) -> _TurnPreparation:
        ...
        if live_web_requested and live_web_mode != "off":
            if payload_sensitivity is Sensitivity.S4:
                prep.live_web_error = "live_web_blocked_for_s4"
            ...
            if payload_sensitivity in {
                ...
                try:
                    prep.live_web_result = await self.live_web_service.search(
                        query=payload.text,
                        mode=cast(
    ...
    async def _finalize_turn(
        ...
        *,
        ...
        """Run gates, persist claims and conversation, audit and build the result."""
        payload_sensitivity = prep.payload_sensitivity
        conversation = prep.conversation
        live_web_result = prep.live_web_result
        ...
        await self.circuit_breaker.increment(user_id=user_id, action_type="turn_process")
        audit_payload = result_payload.model_dump(mode="json")
        audit_payload["provider"] = provider_used
        audit_payload["classifier_source"] = prep.classification.source
        audit_payload["local_classifier_available"] = prep.classification.local_classifier_available
        audit_payload["live_web_requested"] = prep.live_web_requested
        audit_payload["live_web_mode"] = prep.live_web_mode
        audit_payload["live_web_strategy"] = live_web_result.strategy if live_web_result else "none"
    ...
    async def _build_llm_messages(
        ...
        *,
        ...
        """Assemble system prompt, user context, history and the new message."""
        provider_is_local = (
            prep.enforce_local and prep.payload_sensitivity in {Sensitivity.S3, Sensitivity.S4}
        ) or (
...
def _error_event_payload(exc: Exception) -> dict[str, Any]:
    """Map service exceptions to structured stream error payloads."""
    if isinstance(exc, LocalProviderUnavailableError):
        return {
    ...
    if isinstance(exc, LiveWebPermissionRequiredError):
        return {
    ...
    if isinstance(exc, CircuitBreakerTrippedError):
        return {"code": "circuit_breaker_tripped", "message": str(exc)}
    if isinstance(exc, ServiceError):
        return {"code": "service_error", "message": str(exc)}
    # Unknown errors stay generic so internals never leak to the client.
    return {"code": "internal_error", "message": "Turn processing failed"}
