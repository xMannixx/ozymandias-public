...
class Settings(BaseSettings):
    ...
    openrouter_daily_token_limit: int = Field(default=0, ge=0)
    ollama_base_url: str = Field(default="http://localhost:11434")
    lmstudio_base_url: str = Field(default="http://localhost:1234/v1")
    #: The deepseek-chat and deepseek-reasoner aliases were retired in July 2026;
    #: thinking is a request parameter on V4, not a separate model.
    deepseek_model: str = Field(default="deepseek-v4-flash")
    gemini_model: str = Field(default="gemini-2.0-flash")
    openai_model: str = Field(default="gpt-4o")
    mistral_model: str = Field(default="mistral-large-latest")
    #: OpenRouter's "~" slugs are rolling aliases, so this default never goes stale.
    openrouter_model: str = Field(default="~openai/gpt-mini-latest")
